...
class SimpleMTL(nn.Module):
  def __init__(self, input_size, output_sizes, hidden_sizes):
    ...
    # self.shared = nn.Sequential(nn.Linear(input_size, hidden_sizes[0]), nn.ReLU())
    # self.shared = nn.Sequential(nn.Dropout(p=1e-2),nn.Linear(input_size, hidden_sizes[0]), nn.ReLU(),nn.Dropout(p=1e-2),nn.Linear(hidden_sizes[0], hidden_sizes[1]), nn.ReLU())
    self.shared = nn.Sequential(nn.Dropout(p=1e-2),nn.Linear(input_size, hidden_sizes[0]), nn.ReLU())
